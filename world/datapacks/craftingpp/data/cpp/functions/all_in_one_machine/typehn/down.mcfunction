...
execute as @s[scores={cppMacType=10}] run replaceitem block ~ ~-1 ~ container.0 minecraft:potion{CustomPotionColor:16777215,HideFlags:63,display:{Name:"{\"translate\":\"item.cpp.bottle_of_salt\"}"},id:"cpp:bottle_of_salt"}

scoreboard players set #random_min cppRandom 1
scoreboard players set #random_max cppRandom 5
function cpp:random
execute as @s[scores={cppMacType=1..9,cppRandom=1}] run replaceitem block ~ ~-1 ~ container.1 minecraft:charcoal{HideFlags:63,id:"cpp:cinder",display:{Name:"{\"translate\":\"item.cpp.cinder\"}"},Enchantments:[{}]}
